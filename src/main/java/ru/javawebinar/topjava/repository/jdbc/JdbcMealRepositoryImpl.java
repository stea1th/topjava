...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal get(int id, int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM meals m " +
                "WHERE m.id=? AND m.user_id=? ", ROW_MAPPER, id, userId);
        return DataAccessUtils.singleResult(meals);
    }

    @Override
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT * FROM meals m " +
                "WHERE m.user_id=? " +
                "ORDER BY 2 DESC ", ROW_MAPPER, userId);
    }

    @Override
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals m " +
                "WHERE m.user_id=? AND m.datetime BETWEEN ? AND ? " +
                "ORDER BY 2 DESC ", ROW_MAPPER, userId, startDate, endDate);
    }
}
